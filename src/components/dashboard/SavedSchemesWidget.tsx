...
export const SavedSchemesWidget = ({ schemes }: SavedSchemesWidgetProps) => {
  return (
    <HoverCard className="bg-white/90 backdrop-blur-[10px] border border-white/20 shadow-[0px_10px_30px_rgba(34,34,34,0.05)] rounded-[20px] transition-all duration-300 p-6 flex flex-col flex-1 h-full">
        ...
        {schemes.length > 0 ? (
          schemes.slice(0, 3).map((scheme, index) => (
            <FadeIn key={scheme.id} delay={index * 0.1} yOffset={10}>
              <div className="group p-4 rounded-xl border border-border/50 hover:border-border transition-colors cursor-pointer bg-white/50">
                <h4 className="text-[14px] font-semibold text-primary line-clamp-1 group-hover:text-black">{scheme.title}</h4>
                <p className="text-[12px] text-muted-foreground mt-1 line-clamp-1">{scheme.ministry}</p>
              </div>
            </FadeIn>
          ))
        ) : (
          <FadeIn delay={0.1} yOffset={10} className="flex-grow flex flex-col items-center justify-center text-center p-4 border border-dashed border-border rounded-xl bg-white/30">
